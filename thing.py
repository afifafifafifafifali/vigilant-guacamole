...
\t1. To send to Victim .
\t2. Beef listens on Port 3000 ,\n\t   so this link should be forwarded to LocalHost:3000 .
	
Just Enter your links in the Script \nScript will do necessary changes required to opt for your Links .
...
if __name__ == '__main__':
    ...
    send_to = raw_input(color((' [?] Enter Address of Link [You are Sending to Victim]: '))
    send_to = send_to.rstrip()
    print color((" [+] Send_To Link  : " + send_to))
    connect_to = raw_input(color((' [?] Enter Address of Link [Your Link will be Connecting to..]: '))
